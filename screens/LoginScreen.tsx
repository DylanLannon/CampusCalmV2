...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      email: email.trim(),
      options: {
        emailRedirectTo: Platform.OS === "web" ? window.location.origin : "campuscalm://",
      },
    });
    ...
    <SafeAreaView style={styles.container}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.inner}>
        <View style={styles.hero}>
          ...
          <Text style={styles.title}>Campus Calm</Text>
          <Text style={styles.subtitle}>Financial wellbeing for students</Text>
        </View>
        ...
        {sent ? (
          ...
          <View style={styles.form}>
            <Text style={styles.formLabel}>Enter your university email</Text>
            <TextInput
              style={styles.input}
        ...
        <View style={styles.footer}>
          <Text style={styles.footerText}>Built on research from Northumbria University</Text>
          <Text style={styles.footerText}>Financial pressure is the #1 anxiety trigger for students</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  footerText: { fontSize: FONTS.size.xs, color: COLORS.gray400, textAlign: "center" },
});